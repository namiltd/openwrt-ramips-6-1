...
#define CHIP_RTL8367B_1			((CHIP_FAMILY_RTL8367B_1 << CHIP_SHIFT) + 0)
#define CHIP_RTL8367R_VB		((CHIP_FAMILY_RTL8367B_1 << CHIP_SHIFT) + 1)
...
static int rtl8367b_init_regs(struct rtl8366_smi *smi)
{
	...
	case CHIP_FAMILY_RTL8367D:
		...
		break;
	case CHIP_FAMILY_RTL8367B_0:
		initvals = rtl8367r_vb_initvals_0;
		count = ARRAY_SIZE(rtl8367r_vb_initvals_0);
		break;
	case CHIP_FAMILY_RTL8367B_1:
				...
				const struct rtl8366_vlan_4k *vlan4k)
{
	...
	if (vlan4k->vid >= RTL8367B_NUM_VIDS ||
	    ...
	    vlan4k->fid > (((smi->chip >> 4) == CHIP_FAMILY_RTL8367D) ? RTL8367D_FIDMAX : RTL8367B_FIDMAX))
		return -EINVAL;
				...
				const struct rtl8366_vlan_mc *vlanmc)
{
	...
	if (index >= RTL8367B_NUM_VLANS ||
	    ...
	    vlanmc->fid > (((smi->chip >> 4) == CHIP_FAMILY_RTL8367D) ? RTL8367D_FIDMAX : RTL8367B_FIDMAX))
		return -EINVAL;
...
static int rtl8367b_detect(struct rtl8366_smi *smi)
{
	...
	if (!chip_name) {
		...
		switch (rlvid) {
		case 0:
			smi->chip = CHIP_RTL8367B_0;
			break;
		case 1:
			smi->chip = CHIP_RTL8367B_1;
			break;
		default:
